...
async function run() {
  try {
    ...
    const { accessKeyId, secretAccessKey, sessionToken } = await getCredentials(oidcAudience, roleArn);
...
async function getCredentials(oidcAudience, roleArn) {
  const idToken = await core.getIDToken(oidcAudience);

  let assumeRoleOutput = "";
  await exec.exec(
    `aws sts assume-role-with-web-identity --role-arn ${roleArn} --role-session-name deploy-s3-javascript-action --web-identity-token ${idToken} --duration-seconds 3600`,
    [],
    {
      listeners: {
        stdout: (data) => {
...
async function createBackupTarGz() {
